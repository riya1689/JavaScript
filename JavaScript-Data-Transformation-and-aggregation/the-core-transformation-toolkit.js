//core transformation toolkit(map, filter, sort, slice)
...
//forEach , for (or), for(on), filter, map, set
...
.filter((item) => item.category === "Electronics") // item = property or key
.sort ((a,b) => b.rating - a.rating)
.slice (0,3)
.map((item) => {
    return {name: item.productName}; //write () => {return  }  if multiple line to execute.or one liner
});
